...
TEST_P(MultiplicationTest, consistency) {
	...
	const perm::ExplicitPermutation p2 = perm::ExplicitPermutation(std::get< 1 >(GetParam()));
...
TEST_P(InvertTest, invert) {
	...
	perm::ExplicitPermutation original        = perm::ExplicitPermutation(std::get< 0 >(GetParam()));
	perm::ExplicitPermutation perm            = original;
	perm::ExplicitPermutation expectedInverse = perm::ExplicitPermutation(std::get< 1 >(GetParam()));
